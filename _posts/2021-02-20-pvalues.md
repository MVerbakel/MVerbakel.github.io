...
These scenarios demonstrate that the better our hypotheses are (i.e. the more likely they are to actually be effective), then the lower our False Discovery Rate is (holding the FPR and power constant). If we are just testing lots of random changes with little thought, it's possible that a high proportion of the trials we approve (or product changes we make) are actually ineffective (and likely not reproducible). To reduce the FDR we can increase the sample size, decreasing the FPR so that fewer trials that are not effective are incorrectly identified as effective, and increasing the power so that more trials that are effective are correctly identified as so. We can also run multiple experiments, incrementally improving on ideas that appear effective so that we also check the reproducibility. 
...
- Building incrementally is also a good way to reduce wasted effort, while also ensuring effects are reproducible.
...
- [Kohavi, R., Tang, D., & Xu, Y. (2020). Trustworthy Online Controlled Experiments: A Practical Guide to A/B Testing, Chapter 17](https://www.cambridge.org/core/books/trustworthy-online-controlled-experiments/D97B26382EB0EB2DC2019A7A7B518F59#fndtn-information)